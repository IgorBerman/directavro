...
import org.apache.hadoop.mapreduce.DirectFileOutputCommitter;
import org.apache.hadoop.mapreduce.DirectFileOutputFormat;
 ...
 */
public abstract class DirectAvroOutputFormatBase<K, V> extends DirectFileOutputFormat<K, V> {
  ...
  protected static CodecFactory getCompressionCodec(TaskAttemptContext context) {
    if (DirectFileOutputFormat.getCompressOutput(context)) {
  ...
  protected OutputStream getAvroFileOutputStream(TaskAttemptContext context) throws IOException {
    Path path = new Path(((DirectFileOutputCommitter)getOutputCommitter(context)).getWorkPath(),
